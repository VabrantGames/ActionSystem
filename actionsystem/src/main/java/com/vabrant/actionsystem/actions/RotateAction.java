...
public class RotateAction extends PercentAction<Rotatable, RotateAction> {
    ...
    public static RotateAction rotateBy(
            Rotatable rotatable, float amount, float duration, Interpolation interpolation) {
    ...
    public static RotateAction setRotation(Rotatable rotatable, float rotation) {
        return obtain().rotateTo(rotation).set(rotatable, 0, null);
    }
    ...
    /** Caps the end value between <i> 0 (inclusive)</i> - <i> 360 (exclusive) </i> for degrees or <i> 0 (inclusive) </i> - <i> 2 *
     * PI (exclusive) </i> for radians.
     *
     * @return This action for chaining. */
