...
public class PercentActionTest {
	...
	@Parameterized.Parameter(0) public float end;

	@Parameterized.Parameter(1) public int duration;

	@Parameterized.Parameters()
	public static Collection<Object[]> data () {
		Object[][] data = new Object[][] {{10, 1}, {5, 2}};
	...
	public PercentTestAction getTestAction () {
		PercentTestAction action = PercentTestAction.set(testClass, end, duration).setLogLevel(ActionLogger.LOGGER_DEBUG);
	...
	public void unmanagedMultiRunTest () {
		...
		PercentTestAction action = PercentTestAction.set(testClass, end, duration).unmanage()
			.setLogLevel(ActionLogger.LOGGER_DEBUG);
	...
	public void setTimeTest () {
		...
		ActionManager manager = new ActionManager();
		PercentTestAction action = PercentTestAction.set(testClass, 10, 1).setLogLevel(ActionLogger.LOGGER_DEBUG);
	...
	/*
	 * The class does not depict an actual implementation of a class that extends PercentAction. It was created for testing
	 * purposes.
	 */
	private static class PercentTestAction extends PercentAction<TestPercentable, PercentTestAction> {
		...
		public static PercentTestAction set (TestPercentable percentable, float end, float duration) {
			return obtain().moveTo(end).set(percentable, duration, Interpolation.linear);
		}
