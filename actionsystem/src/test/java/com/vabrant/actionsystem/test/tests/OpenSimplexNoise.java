...
/** K.jpg's OpenSimplex 2, faster variant <br>
...
public class OpenSimplexNoise {
    ...
    /** 2D Simplex noise, with Y pointing down the main diagonal. Might be better for a 2D sandbox style game, where Y is vertical.
     * Probably slightly less optimal for heightmaps or continent maps, unless your map is centered around an equator. It's a
     * subtle difference, but the option is here to make it an easy choice. */
    ...
    /** 3D OpenSimplex2 noise, with better visual isotropy in (X, Y). Recommended for 3D terrain and time-varied animations. The Z
     * coordinate should always be the "different" coordinate in whatever your use case is. If Y is vertical in world coordinates,
     * call noise3_ImproveXZ(x, z, Y) or use noise3_XZBeforeY. If Z is vertical in world coordinates, call noise3_ImproveXZ(x, y,
    ...
    /** 3D OpenSimplex2 noise, with better visual isotropy in (X, Z). Recommended for 3D terrain and time-varied animations. The Y
     * coordinate should always be the "different" coordinate in whatever your use case is. If Y is vertical in world coordinates,
     * call noise3_ImproveXZ(x, Y, z). If Z is vertical in world coordinates, call noise3_ImproveXZ(x, Z, y) or use
     * noise3_ImproveXY. For a time varied animation, call noise3_ImproveXZ(x, T, y) or use noise3_ImproveXY. */
        ...
        for (int l = 0; ; l++) {
            ...
            if (ax0 >= ay0 && ax0 >= az0) {
                ...
                if (b > 1) {
                    b -= 1;
                    value +=
                            (b * b) * (b * b) * grad(seed, xrbp - xNSign * PRIME_X, yrbp, zrbp, xri + xNSign, yri, zri);
                }
                ...
                if (b > 1) {
                    b -= 1;
                    value +=
                            (b * b) * (b * b) * grad(seed, xrbp, yrbp - yNSign * PRIME_Y, zrbp, xri, yri + yNSign, zri);
                }
            } else {
                float b = a + az0 + az0;
                if (b > 1) {
                    b -= 1;
                    value +=
                            (b * b) * (b * b) * grad(seed, xrbp, yrbp, zrbp - zNSign * PRIME_Z, xri, yri, zri + zNSign);
                }
            }
    ...
    /** 4D OpenSimplex2 noise, with XYZ oriented like noise3_ImproveXY and W for an extra degree of freedom. W repeats eventually.
     * Recommended for time-varied animations which texture a 3D object (W=time) in a space where Z is vertical */
    ...
    /** 4D OpenSimplex2 noise, with XYZ oriented like noise3_ImproveXZ and W for an extra degree of freedom. W repeats eventually.
     * Recommended for time-varied animations which texture a 3D object (W=time) in a space where Y is vertical */
    ...
    /** 4D OpenSimplex2 noise, with XYZ oriented like noise3_Fallback and W for an extra degree of freedom. W repeats eventually.
     * Recommended for time-varied animations which texture a 3D object (W=time) where there isn't a clear distinction between
    ...
    /** 4D OpenSimplex2 noise, with XY and ZW forming orthogonal triangular-based planes. Recommended for 3D terrain, where X and Y
     * (or Z and W) are horizontal. Recommended for noise(x, y, sin(time), cos(time)) trick. */
    ...
    private static float grad(
            long seed, long xsvp, long ysvp, long zsvp, long wsvp, float dx, float dy, float dz, float dw) {
